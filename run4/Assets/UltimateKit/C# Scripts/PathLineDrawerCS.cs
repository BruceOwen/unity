...
public class PathLineDrawerCS : MonoBehaviour {
	...
	private void SetCurrentPatchCPs()
	{
		...
		foreach (Transform child in CPs_Group)//得到子对象数量
			i++;
		int NumCPs = i;
	...
	private Vector3[] PathControlPointGenerator(Vector3[] path)
	{
			...
			System.Array.Copy(suppliedPath,0,vector3s,1,suppliedPath.Length);//将位点复制给新数组的第二到倒数第二个
